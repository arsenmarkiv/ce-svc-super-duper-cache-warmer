...
# def make_api_request(url, method="GET", data=None, verify_ssl=False, api_version='33', access_token=None):
...
#     res = requests.request(method, full_url, data=json.dumps(data), headers=headers, verify=verify_ssl)
#
#     if res.status_code == 204:
#         return {'data': []}
#     if res.status_code != 200:
#         message = f'Non-200 status code ({res.status_code}) while making api request {full_url}: {res.reason}'
#         raise exceptions.APIResultException(message, res.status_code)
#
#     try:
#         return res.json()
#     except ValueError:
#         raise exceptions.APIResultException(f'Failed to decode dist-analytics result: {res.text}')
...
async def call_summary_api(period: str, threads: int):
    ...
    # run_with_executor('summary', 'POST', json, threads)

    # for single_json in json_data:
    #     make_api_request('summary', 'POST', single_json)


# TODO: add GitHub files parsing functionality
async def parse_resources(period):
    file_name = period + '.json'

    project_path = os.path.abspath(os.path.split(sys.argv[0])[0])
    path = os.path.join(project_path, 'resources', file_name)
